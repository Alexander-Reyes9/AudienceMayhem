...
public class VICreateRoomScript : MonoBehaviour
{
    ...
    public async void CreateRoomAsync(string s, int maxConnections)
    {
        ...
        {
            Debug.Log("There IS a network object attached to the gameobject and testing");
        }
        MyPlayer myPlayerObjScript = myPlayerObj.GetComponent<MyPlayer>();
        myPlayerObjScript.IncrementRoomCountServerRpc();
        //Ian's Changes
        NetworkManager.Singleton.SceneManager.LoadScene("VIAudFinal", LoadSceneMode.Single);
