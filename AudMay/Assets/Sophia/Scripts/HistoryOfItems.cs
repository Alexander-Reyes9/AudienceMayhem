...
public class HistoryOfItems : MonoBehaviour
{
    ...
    public List<int> pickupsCount;
    public List<int> powerupsCount;
    ...
    void Start()
    {
        ...
        powerupsArr = GameObject.FindGameObjectsWithTag("effect");
        ...
        Debug.Log("At start " + pickupsList.Count);
    ...
    public void AddNewGameObject(GameObject i)
    {
        ...
        switch(tag)
        {
            case "Pickup":
                ...
                {
                    Debug.Log("Before if statemenet (checkHistory) runs");
                    if (CheckHistory(item))
                    {
                        pickupsList.Add(i);
                        Debug.Log("object added to pickupsList");
                        pickupsCount.Add(1);
                    }
                }
                break;
    ...
    {
        int count = 0;
        Debug.Log($"is this list null {pickupsList}");
    ...
    private void SetStrings()
    {
        ...
        for (int i = 0; i < pickupsList.Count; i++)
        {
            ...
            if (item.IndexOf("(") > 0)
            {
                strItem = item.Remove(item.IndexOf("("));
            }
    ...
    private void SetHistory()
    {
        ...
        Debug.Log("SetHistory called");
    }

    //adds pickups, powerups, and weapons to inventory arraylist on trigger
    private void OnTriggerEnter(Collider collider)
    {
        if (collider.tag.Equals("Pickup") || collider.tag.Equals("effect"))// || collider.tag.Equals("Weapon"))
        {
